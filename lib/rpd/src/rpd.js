...
if (!Kefir)
  throw new Error(
    "Kefir.js (https://github.com/rpominov/kefir) is required for Rpd to work"
  );
...
var Rpd = (function () {
  ...
  var noderenderers = {};
  var NODE_RENDERER_PROPS = ["prepare", "size", "first", "always"];
  var channelrenderers = {};
  var CHANNEL_RENDERER_PROPS = ["prepare", "show", "edit"];
  ...
  rpdEvent["network/add-patch"].onValue(function (patch) {
    rpdEvents.plug(patch.events);
  });
  ...
  function /*Rpd.*/ addClosedPatch(arg0, arg1) {
    var name = !is_object(arg0) ? arg0 : undefined;
    var def = arg1 || arg0;
  ...
  function Patch(name, def) {
    ...
    var renderStream = Kefir.combine(
      [this.events],
      [
        this.renderQueue.scan(function (storage, rule) {
          var alias = rule.alias,
            target = rule.target,
    ...
    Kefir.combine(
      [this.projections],
      [this.event["patch/set-inputs"], this.event["patch/set-outputs"]]
      ...
      for (var i = 0; i < inputs.length; i++) {
        ...
        inlet.event["inlet/update"].onValue(
          (function (input) {
      ...
      for (i = 0; i < outputs.length; i++) {
        outlet = node.addOutlet(
          outputs[i].type,
          outputs[i].alias,
          outputs[i].def,
          outputs[i].render
        );
        outputs[i].event["outlet/update"].onValue(
          (function (outlet) {
            return function (value) {
              outlet.send(value);
            };
  ...
  Patch.prototype.render = function (aliases, targets, config) {
    ...
    for (var i = 0, il = aliases.length, alias; i < il; i++) {
      for (var j = 0, jl = targets.length, target; j < jl; j++) {
        alias = aliases[i];
        target = targets[j];
        if (!renderer_registry[alias])
          report_system_error(
            this,
            "patch",
            "Renderer '" + alias + "' is not registered"
          );
  ...
  function Node(type, patch, def, render, callback) {
    ...
    if (!type_def)
      report_system_error(
        this,
        "node",
        "Node type '" + this.type + "' is not registered!"
      );
    this.def = join_definitions(NODE_PROPS, def, type_def);

    this.render = join_render_definitions(
      NODE_RENDERER_PROPS,
      render,
    ...
    if (this.def.process) {
      ...
      process = process
        .bufferBy(this.event["node/is-ready"])
        .take(1)
        .flatten()
        .concat(process);
      ...
      process = process
        .scan(
          ...
          { inlets: { prev: {}, cur: {} }, outlets: {} }
        )
        .changes();

      // filter cold inlets, so the update data will be stored, but process event won't fire
      process = process.filter(function (data) {
        return !data.source.def.cold;
      });

      process.onValue(function (data) {
        // call a node/process event using collected inlet values
        var outlets_vals = process_f.bind(node)(
          data.inlets.cur,
          data.inlets.prev
        );
        node.event["node/process"].emit({
          inlets: data.inlets.cur,
          outlets: outlets_vals,
        });
    ...
    if (this.def.prepare)
      this.def.prepare.bind(this)(this.inlets, this.outlets);
  ...
  function Inlet(type, node, alias, def, render) {
    ...
    if (!type_def)
      report_system_error(
        this,
        "inlet",
        "Channel type '" + this.type + "' is not registered!"
      );
    ...
    if (this.def.accept)
      updates = updates.flatten(
        function (v) {
          if (this.def.accept(v)) {
            return [v];
          } else {
            orig_updates.error(make_silent_error(this, "inlet"));
            return [];
          }
        }.bind(this)
      );
  ...
  function Outlet(type, node, alias, def, render) {
    ...
    if (!this.alias)
      report_error(this, "outlet", "Outlet should have an alias");
  ...
  Outlet.prototype.connect = function (inlet) {
    if (!inlet.allows(this)) {
      report_error(
        ...
        "Outlet of type '" +
          this.type +
          "' is not allowed to connect to inlet of type '" +
          inlet.type +
          "'"
      );
    }
  ...
  function Link(outlet, inlet, label) {
    ...
    this.enabled = Kefir.merge([
      this.event["link/disable"].map(ƒ(false)),
      this.event["link/enable"].map(ƒ(true)),
    ]).toProperty(ƒ(true));
  ...
  function injectKefirEmitter() {
    Kefir.emitter = function () {
      var e,
        stream = Kefir.stream(function (_e) {
          e = _e;
          return function () {
            e = undefined;
          };
  ...
  function join_render_definitions(keys, user_render, type_render) {
    ...
    for (var render_type in type_render) {
      result[render_type] = join_definitions(
        keys,
        user_render[render_type],
        type_render[render_type]
      );
    }
    for (var render_type in user_render) {
      if (!result[render_type] && !type_render[render_type])
        result[render_type] = user_render[render_type];
    }
    return result;
  }
  ...
  function make_error(subject, subject_name, message, is_system, is_silent) {
    return {
      type: subject_name + "/error",
      system: is_system || false,
      subject: subject,
      message: message,
      silent: is_silent || false,
    };
  }

  function report_error(subject, subject_name, message, is_system) {
    rpdEvents.plug(
      Kefir.constantError(make_error(subject, subject_name, message, is_system))
    );
  }
  ...
  function get_style(name, renderer) {
    if (!name)
      report_system_error(
        null,
        "network",
        "Unknown style requested: '" + name + "'"
      );
    if (!styles[name])
      report_system_error(
        null,
        "network",
        "Style '" + name + "' is not registered"
      );
    ...
    if (!style)
      report_system_error(
        null,
        "network",
        "Style '" + name + "' has no definition for '" + renderer + "' renderer"
      );
    return style;
  }
  ...
  stringify.patch = function (patch) {
    return (
      "[ Patch" + (patch.name ? " '" + patch.name + "'" : " <Unnamed>") + " ]"
    );
  };
  ...
  function autoStringify(value) {
    if (value instanceof Patch) {
      return stringify.patch(value);
    }
    if (value instanceof Node) {
      return stringify.node(value);
    }
    if (value instanceof Inlet) {
      return stringify.inlet(value);
    }
    if (value instanceof Outlet) {
      return stringify.outlet(value);
    }
    if (value instanceof Link) {
      return stringify.link(value);
    }
    return "<?> " + value;
  }
