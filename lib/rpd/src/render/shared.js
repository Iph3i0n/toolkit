module.exports = function (Rpd) {
  Rpd.Render = (function () {
    ...
    GridPlacing.prototype.nextPosition = function (node, size, limits) {
      ...
      var newRect = {
        x: lastRect ? lastRect.x : edgePadding.horizontal,
        y: lastRect
          ? lastRect.y + lastRect.height + boxPadding.vertical
    ...
    DragAndDrop.prototype.add = function (handle, spec) {
      ...
      var start = spec.start,
        ...
        drag = spec.drag;
      Kefir.fromEvents(handle.node(), "mousedown")
        .map(extractPos)
        .map(style.getLocalPos)
        .flatMap(function (pos) {
          var initPos = start(),
            diffPos = { x: pos.x - initPos.x, y: pos.y - initPos.y };
          var moveStream = Kefir.fromEvents(canvas.node(), "mousemove")
            ...
            .map(style.getLocalPos)
            .map(function (absPos) {
              return { x: absPos.x - diffPos.x, y: absPos.y - diffPos.y };
            })
            .toProperty(function () {
              return initPos;
            });
    ...
    VLinks.prototype.clear = function () {
      this.vlinks = [];
    };
    ...
    function mergeConfig(user_conf, defaults) {
      if (user_conf) {
        ...
        Object.keys(user_conf).forEach(function (prop) {
          merged[prop] = user_conf[prop];
        });
        return merged;
