    ...
    return function (networkRoot, userConfig) {
      ...
      return {
        ...
        "patch/is-ready": function (update) {
          ...
          canvas = d3
            .select(style.createCanvas(patch, networkRoot).element)
            .classed("rpd-canvas", true);

          if (config.fullPage)
            canvas.style("height", docElm.property("clientHeight") + "px");

          // resize network root on window resize
          if (config.fullPage)
            updateCanvasHeightOnResize(window, document, networkRoot, canvas);
          ...
          δ(canvas, { patch: update.patch });

          tree.patches[patch.id] = canvas;
          ...
          if (config.nodeMovingAllowed)
            dnd = new Render.DragAndDrop(canvas, style);

          Kefir.fromEvents(canvas.node(), "selectstart").onValue(
            preventDefault
          );
        },
        ...
        "patch/refer": function (update) {
          ...
          δ(nodeBox).processTarget.text(
            "[" + (update.target.name || update.target.id) + "]"
          );
        ...
        "patch/add-node": function (update) {
          ...
          δ(nodeBox, {
            inletsTarget: nodeElm.select(".rpd-inlets-target"),
          ...
          if (render.first)
            subscribeUpdates(
              ...
              render.first.bind(node)(
                nodeElm.select(".rpd-process-target").node(),
          ...
          var nodeSize = styledNode.size,
            nodePos = placing.nextPosition(node, nodeSize, {
              width: limitSrc.node().offsetWidth,
              height: limitSrc.node().offsetHeight,
            });

          nodeElm
            .select(".rpd-body")
            .style("min-width", Math.floor(nodeSize.width) + "px")
            .style("height", Math.floor(nodeSize.height) + "px"); // height is min-height for table cells
        ...
        "node/move": function (update) {
          ...
          nodeBox
            .style("left", Math.floor(position[0]) + "px")
            .style("top", Math.floor(position[1]) + "px");
          δ(nodeBox).pos = { x: position[0], y: position[1] };
        },
        ...
        "node/add-inlet": function (update) {
          ...
          var inletsTarget = δ(tree.nodes[update.node.id]).inletsTarget;
          var render = update.render;
        ...
        "node/add-outlet": function (update) {
          ...
          var outletsTarget = δ(tree.nodes[update.node.id]).outletsTarget;
          var render = update.render;
