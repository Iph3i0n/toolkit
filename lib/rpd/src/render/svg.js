module.exports = function (Rpd) {
    ...
    return function (networkRoot, userConfig) {
      ...
      return {
        ...
        "patch/is-ready": function (update) {
          ...
          if (config.fullPage) {
            svg
              .attr("width", "100%" /*docElm.property('clientWidth')*/)
              .attr("height", docElm.property("clientHeight"));
            svg
              .select(".rpd-background")
              .attr("width", "100%" /*docElm.property('clientWidth')*/)
              .attr("height", docElm.property("clientHeight"));
          }

          var patchCanvas = svg
            .append(ƒ(style.createCanvas(patch, networkRoot.node()).element))
            .classed("rpd-style-" + config.style, true)
            .classed(
              "rpd-values-" +
                (config.valuesOnHover ? "on-hover" : "always-shown"),
              true
            )
            .classed("rpd-show-boxes", config.showBoxes);

          δ(patchCanvas, update.patch);
          ...
          if (config.fullPage)
            updateCanvasHeightOnResize(
              window,
              document,
              svg,
              svg.select(".rpd-background")
            );
          ...
          if (config.nodeMovingAllowed)
            dnd = new Render.DragAndDrop(svg, style);

          Kefir.fromEvents(svg.node(), "selectstart").onValue(preventDefault);
        },

        "patch/open": function (update) {
          if ((config.closeParent || config.fullPage) && update.parent)
            update.parent.close();
          ...
          if (style.onPatchSwitch)
            style.onPatchSwitch(currentPatch, newCanvas.node());
        },
        ...
        "patch/refer": function (update) {
          ...
          δ(nodeBox)
            .processTarget.append(ƒ(_createSvgElement("text")))
          ...
          Kefir.fromEvents(δ(nodeBox).processTarget.node(), "click").onValue(
        ...
        "patch/add-node": function (update) {
          ...
          var placing = tree.patchToPlacing[update.patch.id],
            ...
            limitSrc = δ(tree.patches[currentPatch.id]);

          var nodeBox = d3
            .select(_createSvgElement("g"))
            .attr("class", "rpd-node-box");
          var styledNode = style.createNode(
            node,
            render,
          ...
          δ(nodeBox, {
            inletsTarget: nodeElm.select(".rpd-inlets"),
            outletsTarget: nodeElm.select(".rpd-outlets"),
            processTarget: nodeElm.select(".rpd-process"),
            position: nodePos,
            size: styledNode.size,
          });

          tree.nodes[node.id] = nodeBox;

          var nodePos = placing.nextPosition(node, styledNode.size, {
            width: limitSrc.width,
          ...
          if (render.first)
            subscribeUpdates(
              node,
              render.first.bind(node)(
                nodeElm.select(".rpd-process").node(),
                node.event["node/configure"]
              )
            );
          ...
          if (!removeButton.empty()) {
            Kefir.fromEvents(
              nodeElm.select(".rpd-remove-button-handle").node(),
              "click"
            )
              .map(stopPropagation)
              .onValue(function () {
          ...
          var patchCanvas = δ(tree.patches[node.patch.id]).canvas;
        ...
        "node/move": function (update) {
          ...
          δ(nodeBox).position = { x: position[0], y: position[1] };
        },
        ...
        "node/process": function (update) {
          ...
          if (render.always) {
            var bodyElm = δ(tree.nodes[node.id]).processTarget.node();
        ...
        "node/add-inlet": function (update) {
          ...
          var nodeData = δ(tree.nodes[update.node.id]);
          ...
          if (!inlet.def.readonly && render.edit) {
            ...
            inletElm
              .select(".rpd-value-holder")
              .append(ƒ(editor.editorElm.node()));
          }
        ...
        "node/add-outlet": function (update) {
          ...
          // listen for clicks in connector and allow to edit links this way
          connectivity.subscribeOutlet(
            outlet,
            outletElm.select(".rpd-connector")
          );
        ...
        "node/remove-inlet": function (update) {
          var inlet = update.inlet;
          var inletData = δ(tree.inlets[inlet.id]);
        ...
        "node/remove-outlet": function (update) {
          var outlet = update.outlet;
          var outletData = δ(tree.outlets[outlet.id]);
        ...
        "inlet/update": function (update) {
          ...
          var valueElm = δ(inletElm).value;

          if (!valueElm.empty()) {
            var valueRepr = inlet.def.show
              ? inlet.def.show(update.value)
              : update.value;
            if (render.show) {
              render.show.bind(inlet)(
                valueElm.node().parentNode || valueElm.node(),
                update.value,
                valueRepr
              );
            } else {
        ...
        "outlet/update": function (update) {
          ...
          var valueElm = δ(outletElm).value;

          if (!valueElm.empty()) {
            var valueRepr = outlet.def.show
              ? outlet.def.show(update.value)
              : update.value;
            if (render.show) {
              render.show.bind(outlet)(
                valueElm.node().parentNode || valueElm.node(),
                update.value,
                valueRepr
              );
        ...
        "outlet/connect": function (update) {
          ...
          var outletData = δ(outletElm);
          var inletData = δ(inletElm);

          if (
            !config.inletAcceptsMultipleLinks &&
            inletData.vlinks.count() === 1
          ) {
          ...
          vlink.construct(config.linkWidth).rotateOI(outlet, inlet);
  ...
  function patchByHash(tree) {
    return function (hash) {
      return δ(tree.patches[hash]).patch;
    };
  }
  ...
  function awaiting(a, b) {
    return Kefir.merge([a.map(ƒ(true)), b.map(ƒ(false))]).toProperty(ƒ(false));
  }
  ...
  var Connectivity = (function () {
    function getLinks(inlet) {
      return δ(tree.inlets[inlet.id]).vlinks;
    }
    ...
    function getConnector(outlet) {
      return δ(tree.outlets[outlet.id]).connector;
    }
    ...
    function Connectivity(canvas, style, config) {
      ...
      (this.inletClicks = Kefir.pool()), (this.outletClicks = Kefir.pool());
    ...
    Connectivity.prototype.subscribeOutlet = function (outlet, connector) {
      ...
      var canvasClicks = this.canvasClicks,
        outletClicks = this.outletClicks,
        inletClicks = this.inletClicks;
      var startLink = this.startLink,
        finishLink = this.finishLink,
        doingLink = this.doingLink;
      ...
      Kefir.fromEvents(connector.node(), "click")
        ...
        .onValue(function (pos) {
          startLink.emit();
          var ghost = new VLink(null, style)
            .construct(config.linkWidth)
            .rotateO(outlet, pos.x, pos.y)
            .noPointerEvents()
            .appendTo(canvas);
          d3.select(ghost.getElement()).classed(
            "rpd-" + outlet.type.replace("/", "-"),
            true
          );
              ...
              Kefir.merge([
                ...
                .onValue(function (success) {
                  ...
                  if (config.inletAcceptsMultipleLinks) {
                    ...
                    removeExistingLink(otherInletLinks);
                  }
  ...
  function ValueEditor(
    ...
    valueOut.onValue(function (value) {
      inlet.receive(value);
    });
    Kefir.combine(
      [
