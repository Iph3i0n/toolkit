module.exports = function (Rpd) {
  ...
  var renderUpdate = {
    "network/add-patch": function (update) {
      return spanWithText([" + ", update.patch.title || "<Unnamed>"]);
    },
    "patch/is-ready": function (update) {
      return spanWithText([update.patch.title || "<Unnamed>"]);
    },
    "patch/open": function (update) {
      return spanWithText([" o ", update.patch.title || "<Unnamed>"]);
    },
    "patch/close": function (update) {
      return spanWithText([" x ", update.patch.title || "<Unnamed>"]);
    },
    ...
    "node/turn-on": function (update) {
      return spanWithText([update.node.def.title]);
    },
    ...
    "node/move": function (update) {
      return spanWithText([update.node.def.title, " => ", update.position]);
    },
    ...
    "outlet/connect": function (update) {
      return spanWithText([
        update.link.outlet.alias +
          " (" +
          update.link.outlet.node.def.title +
          ")",
        " => ",
  ...
  Rpd.visualHistory = function (target, type) {
    ...
    Rpd.events.onValue(function (update) {
      ...
      li.append("span")
        .classed("update-type", true)
        .text((updateTitle[updateType] || updateType) + ":");
