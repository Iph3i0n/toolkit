module.exports = function (Rpd) {
  ...
  Rpd.channeltype("anm/vectors", {
    adapt: S(VECTORS),
    show: stringify,
    accept: accept(VECTORS),
  });
  Rpd.channeltype("anm/colors", {
    adapt: S(COLORS),
    show: stringify,
  ...
  Rpd.channeltype("anm/elements", {
    adapt: S(ELEMENTS),
    show: stringify,
    accept: accept(ELEMENTS),
  });
  Rpd.channeltype("anm/force", {
    show: function (v) {
      return v ? "[Force]" : "None";
    },
    accept: function (v) {
  ...
  Rpd.nodetype("anm/color", {
    ...
    process: function (inlets) {
      return {
        color: Spread.join(
          [inlets.red, inlets.green, inlets.blue, inlets.alpha],
          COLORS,
          function (r, g, b, a) {
            return (
              "rgba(" +
  ...
  Rpd.nodetype("anm/vector", {
    ...
    process: function (inlets) {
      return {
        vector: Spread.join([inlets.x, inlets.y], VECTORS, function (x, y) {
  ...
  Rpd.nodetype("anm/primitive", {
    ...
    inlets: {
      pos: {
        type: "anm/vectors",
        default: Spread.of(new Vector(0, 0), VECTORS),
      },
      color: {
        type: "anm/colors",
        default: Spread.of("rgba(255,60,60,1)", COLORS),
      },
      //'stroke': { type: 'anm/colors', default: 'transparent'    },
      size: {
        type: "anm/vectors",
        default: Spread.of(new Vector(15, 15), VECTORS),
      },
            ...
            return function (elm) {
              ...
              switch (inlets.type) {
                case "dot":
                  ...
                  break;
                case "rect":
                  elm.rect(size.x, size.y);
                  break;
                case "oval":
                  elm.oval(size.x, size.y);
                  break;
                case "triangle":
                  elm.triangle(size.x, size.y);
                  break;
              }
              elm.fill(color);
              //elm._mass = mass;
  ...
  Rpd.nodetype("anm/particles", {
    ...
    inlets: {
      ...
      force: { type: "anm/force", default: function (t) {} }, // force === function(life_t) => Vector
    ...
    process: function (inlets) {
      ...
      return {
        system: Spread.join(
          [inlets.particle, Spread.of(inlets.force, FORCES)],
          ELEMENTS,
