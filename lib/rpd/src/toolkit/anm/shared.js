...
Spread._makeIterator = function (iter_f) {
  return iter_f();
};
Spread._makeStream = function (iter_f, signal) {
  var next = iter_f();
  if (signal) {
    return signal.map(next).takeWhile(function (v) {
      return v !== Spread.STOP;
    });
  } else {
    return Kefir.stream(function (emitter) {
      var v;
      while ((v = next()) !== Spread.STOP) {
        emitter.emit(v);
      }
      emitter.end();
    });
