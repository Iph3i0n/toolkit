module.exports = function (Rpd) {
  ...
  var view = new PdView(defaultSize, function () {
    ...
    return {
      ...
      first: function (bodyElm) {
        ...
        d3.select(bodyElm).call(function (body) {
          body.append(ƒ(path.node()));
          body.append(ƒ(text.node()));
          body.append(ƒ(handle.node()));
        });
    ...
    return {
      ...
      first: function (bodyElm) {
        ...
        view.addEditor(bodyElm, text.node(), function (value) {
          lastValue = PdValue.extract(value);
        });
        view.addValueSend(bodyElm, this, "receive", function () {
  ...
  function isToggled(val) {
    return val.getByIndex(0) > 0;
  }
  function toggledOff() {
    return PdValue.from([0]);
  }
  function toggledOn() {
    return PdValue.from([1]);
  }
  function switchToggle(from) {
    return isToggled(from) ? toggledOff() : toggledOn();
  }
  Rpd.noderenderer("wpd/toggle", "svg", function (node) {
    var rect;
    var mark;
    var size = { width: defaultSize.height, height: defaultSize.height };
    ...
    return {
      ...
      first: function (bodyElm) {
        ...
        return {
          receive: {
            valueOut: changes.toProperty(function () {
              return toggledOff();
            }),
          },
        };
    ...
    return {
      ...
      first: function (bodyElm) {
        rect = d3
          ...
          .attr("height", size.height);
        circle = d3
          .select(_createSvgElement("circle"))
          .attr("cx", size.width / 2)
          .attr("cy", size.width / 2)
          .attr("r", size.width / 2);
        view.addSelection(bodyElm, node);
        view.addValueSend(bodyElm, this, "receive", function () {
          return PdValue.bang().get();
        });
        d3.select(bodyElm).call(function (body) {
          body.append(ƒ(rect.node()));
          body.append(ƒ(circle.node()));
        });
      },
    };
  });
  ...
  Rpd.noderenderer("wpd/toolbar", "svg", function (node) {
    ...
    return {
      size: { width: mWidth + 100, height: mHeight },
      first: function (bodyElm) {
        var group = d3
          .select(_createSvgElement("g"))
        ...
        group
          .append("rect")
          .classed("rpd-wpd-toolbar-border", true)
          .attr("height", mHeight)
          .attr("width", mWidth + 100)
          .attr("rx", 5)
          .attr("ry", 5);
        ...
        group.append("g").call(function (editGroup) {
          editGroup
            .attr("transform", "translate(12, " + mHeight / 2 + ")")
          ...
          editGroup.append(ƒ(outerCircle.node()));
          editGroup
            .append(ƒ(innerCircle.node()))
          ...
          var keyLabel = d3
            .select(_createSvgElement("text"))
            .text("(" + editModeKeyLabel + ")")
            .attr("x", 55)
            .attr("y", 1)
            .classed("rpd-wpd-key-label", true);
          editGroup.append(ƒ(text.node()));
          editGroup.append(ƒ(keyLabel.node()));
        ...
        group.append("g").call(function (buttons) {
          ...
          Object.keys(commandToType).forEach(function (command, idx) {
            ...
            if (commandToType[command])
              view.addNodeAppender(button.node(), nodeType, node.patch);
            buttons.append(ƒ(button.node()));
          });
        });

        d3.select(bodyElm).append(ƒ(group.node()));
      },
    };
  });
  ...
  Rpd.noderenderer("wpd/edit-switch", "svg", function (node) {
    return {
      first: function (bodyElm) {
        ...
        var outerCircle = d3.select(_createSvgElement("circle")).attr("r", 7);
        var innerCircle = d3.select(_createSvgElement("circle")).attr("r", 5);
        group.append(ƒ(outerCircle.node()));
        group.append(ƒ(innerCircle.node())).style("pointer-events", "none");
        var text = d3
          .select(_createSvgElement("text"))
          .attr("x", 10)
          .text("Edit mode");
        group.append(ƒ(text.node()));
    ...
    return {
      first: function (bodyElm) {
        ...
        Kefir.fromEvents(ch0switch.node(), "click")
          .toProperty(function () {
            return model.isAudioChannelOn(0);
          })
          ...
          .onValue(function (val) {
            ...
            ch0switch
              .classed("rpd-wpd-channel-on", val)
              .text("CH0:" + (val ? "✔" : "✘"));
          });
        d3.select(bodyElm).append(ƒ(ch0switch.node()));
        d3.select(bodyElm).append(
          ƒ(d3.select(_createSvgElement("text")).text("/").attr("x", 35).node())
        );
        var ch1switch = d3
          .select(_createSvgElement("text"))
          .attr("x", 45)
        ...
        Kefir.fromEvents(ch1switch.node(), "click")
          .toProperty(function () {
            return model.isAudioChannelOn(1);
          })
          .scan(function (prev, next) {
            return !prev;
          })
