module.exports = function (Rpd) {
  ...
  Rpd.channeltype("util/time", {
    ...
    show: function (val) {
      return Math.floor(val / 10) / 100 + "s";
    },
  });
  ...
  Rpd.nodetype("util/random", function () {
    return {
      ...
      process: function (inlets) {
        return {
          random: Math.floor(
            inlets.min + Math.random() * (inlets.max - inlets.min)
          ),
        };
      },
    };
  ...
  Rpd.nodetype("util/comment", {
    inlets: {
      text: { type: "core/any", hidden: true },
  ...
  Rpd.nodetype("util/metro", function () {
    ...
    return {
      title: "metro",
      inlets: {
        enabled: { type: "util/boolean", default: true },
  ...
  Rpd.nodetype("util/knob", {
    ...
    process: function (inlets) {
      return { number: adaptToState(inlets, inlets.knob) };
    },
  });
  ...
  Rpd.nodetype("util/dial", {
    ...
    process: function (inlets) {
      return { number: Math.floor(adaptToState(inlets, inlets.dial)) };
    },
  });
  ...
  Rpd.nodetype("util/knobs", {
    ...
    process: function (inlets) {
      return {
        numbers: inlets.submit
          ? inlets.submit.map(function (num) {
...
*/
  ...
  Rpd.nodetype("util/*", {
    title: "*",
    inlets: { a: { type: "util/number" }, b: { type: "util/number" } },
