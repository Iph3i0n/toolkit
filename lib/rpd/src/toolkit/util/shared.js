var RpdUtils = (function () {
  function adaptToState(state, value) {
    return (
      Math.floor((state.min + (state.max - state.min) * value) * 100) / 100
    );
  }
  ...
  function NodeList(conf) {
    ...
    this.clearingEvents = Kefir.fromEvents(
      this.clearSearchButton.node(),
      "click"
    ).onValue(
      function () {
        this.selectNothing();
    ...
    for (var i = 0; i < listElements.length; i++) {
      ...
      listElements[i].prev =
        i > 0 ? listElements[i - 1] : listElements[listElements.length - 1];
      listElements[i].next =
        i < listElements.length - 1 ? listElements[i + 1] : listElements[0];
    }
  ...
  NodeList.prototype.addOnClick = function () {
    ...
    listElements.forEach(function (elmData) {
      ...
      Kefir.fromEvents(li.node(), "click").onValue(function () {
    ...
    Kefir.fromEvents(search.node(), "input")
      ...
      .onValue(function (searchString) {
        ...
        nodeList.listElements.forEach(function (elmData) {
          ...
          if (index >= 0) {
            nodeList.setVisible(elmData);
          } else {
            nodeList.setInvisible(elmData);
          }
          elmData.visible = index >= 0;
          if (elmData.visible) nodeList.currentlyVisible++;
        });
        nodeList.recalculateSize(
        ...
        return Kefir.fromEvents(document.body, "keyup")
          ...
          .takeUntilBy(
            Kefir.merge([
              ...
              Kefir.fromEvents(document.body, "click")
                .filter(function (evt) {
              ...
              nodeList.clearingEvents.map(function () {
                return false;
              }) /*,
                                          Kefir.fromEvents(search.node(), 'click')*/,
            ])
              .take(1)
              .onValue(function (doAdd) {
          ...
          .onValue(function (key) {
            ...
            search.node().blur();
            if (key === "up") {
              var current = nodeList.selected
                ? nodeList.selected.prev
