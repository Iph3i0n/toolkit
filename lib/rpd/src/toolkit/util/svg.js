module.exports = function (Rpd) {
  ...
  function htmlNode(name) {
    return document.createElementNS(d3.namespaces.xhtml, name);
  }
  ...
  Rpd.channelrenderer("util/number", "svg", {
    ...
    edit: function (target, inlet, valueIn) {
      ...
      return Kefir.fromEvents(valInput, "change").map(function () {
  ...
  Rpd.channelrenderer(
    "util/wholenumber",
    "svg",
    Rpd.allChannelRenderers["util/number"]["svg"]
  );
  ...
  Rpd.noderenderer("util/log", "svg", function () {
    ...
    return {
      ...
      always: function (bodyElm, inlets, outlets) {
        ...
        textElm.text(
          savedValues.length > 0 ? "..." + savedValues.join(", ") + "." : "..."
        );
      },
    };
  });
  ...
  Rpd.noderenderer("util/bang", "svg", {
    ...
    first: function (bodyElm) {
      ...
      return {
        trigger: {
          valueOut: circleClicks.map(function () {
            return {};
          }),
        },
      };
  ...
  function createKnob(state, conf) {
    ...
    return {
      init: function (parent, valueIn) {
        ...
        d3.select(parent).call(function (bodyGroup) {
          face = bodyGroup
            .append("circle")
            .attr("r", conf.radius)
        ...
        Kefir.fromEvents(parent, "mousedown")
          ...
          .flatMap(function () {
            ...
            var values = Kefir.fromEvents(document.body, "mousemove")
              //.throttle(16)
              ...
              .map(function (event) {
                ...
                return {
                  x: event.clientX - (faceRect.left + conf.radius),
            ...
            values.last().onValue(function (val) {
              ...
              handGhost
                .attr(
                  "transform",
                  "rotate(" + adaptAngle(state, lastValue) + ")"
                )
                .style("visibility", "hidden");
  ...
  function initKnobInGroup(knob, nodeRoot, id, count, width) {
    ...
    d3.select(nodeRoot)
      ...
      .attr(
        "transform",
        "translate(" + (id * width + width / 2 - (count * width) / 2) + ",0)"
      )
  ...
  Rpd.noderenderer("util/knob", "svg", function () {
    ...
    return {
      size: { width: defaultKnobConf.width, height: defaultKnobConf.height },
      first: function (bodyElm, configurationIn) {
        var submit = knob.init(
          bodyElm,
          configurationIn
            .filter(function (c) {
              return c.hasOwnProperty("knob");
            })
            .map(function (c) {
              return c.knob;
            })
  ...
  Rpd.noderenderer("util/dial", "svg", function () {
    ...
    var myKnobConf = {
      ...
      adaptValue: function (value) {
        return Math.floor(value);
      },
    };
    var knob = createKnob(state, myKnobConf);
    return {
      size: { width: myKnobConf.width, height: myKnobConf.height },
      first: function (bodyElm, configurationIn) {
        var submit = knob.init(
          bodyElm,
          configurationIn
            .filter(function (c) {
              return c.hasOwnProperty("dial");
            })
            .map(function (c) {
              return c.dial;
            })
  ...
  Rpd.noderenderer("util/knobs", "svg", function () {
    ...
    return {
      size: {
        width: count * defaultKnobConf.width,
        height: defaultKnobConf.height,
      },
      //pivot: { x: 0, y: 0.5 },
      first: function (bodyElm) {
        ...
        valueOut = Kefir.combine(
          knobs.map(function (knob, i) {
            return initKnobInGroup(
              knob,
              nodeRoot,
              i,
              count,
              defaultKnobConf.width
  ...
  function mousePosNodeRenderer() {
    ...
    var xPosStream = Kefir.fromEvents(document.body, "mousemove")
      .throttle(16)
      .map(function (evt) {
        return evt.x;
      });
    var yPosStream = Kefir.fromEvents(document.body, "mousemove")
      .throttle(16)
      .map(function (evt) {
        return evt.y;
      });
    return {
      ...
      always: function (bodyElm, inlets, outlets) {
        if (Number.isNaN(inlets.x) || Number.isNaN(inlets.y) || !inlets.y)
          return;
        var center = dirCircle.getBoundingClientRect();
        var angle = Math.atan2(
          inlets.y - (center.top + radius),
        ...
        posText.innerHTML = posText.innerText =
          "<" + inlets.x + ":" + inlets.y + ">";
      },
    };
  }
    ...
    first: function (bodyElm) {
      ...
      var bodyGroup = d3
        ...
        .attr(
          "transform",
          "translate(" +
            (-1 * nodeListSize.width) / 2 +
            "," +
            (-1 * nodeListSize.height) / 2 +
            ")"
        );
      var searchGroup = bodyGroup
        .append("g")
        .classed("rpd-nodelist-search", true)
      ...
      var nodeList = new NodeList({
        ...
        buildList: function () {
          ...
          var foreignDiv = bodyGroup
            .append(ƒ(svgNode("foreignObject")))
          ...
          nodeListSvg = foreignDiv
            .append(ƒ(svgNode("svg")))
            .classed("rpd-nodelist-list", true)
            .attr("width", nodeListSize.width - 12 + "px");
          var lastY = 0;

          nodeListSvg.append("g").call(function (g) {
            Object.keys(nodeTypesByToolkit).forEach(function (toolkit) {
              var toolkitGroup = g
                .append("g")
                .classed("rpd-nodelist-toolkit", true)
                .attr("transform", "translate(0," + lastY + ")")
                .call(function (g) {
                  if (toolkitIcons[toolkit])
                    g.append("text")
                      .attr("class", "rpd-nodelist-toolkit-icon")
                      .text(toolkitIcons[toolkit]);
                  g.append("text")
                    .attr("class", "rpd-nodelist-toolkit-name")
                    .text(toolkit);
                });
              ...
              g.append("g")
                .classed("rpd-nodelist-nodetypes", true)
                .call(function (g) {
                  nodeTypesByToolkit[toolkit].types.forEach(function (
                    nodeTypeDef
                  ) {
                    var nodeType = nodeTypeDef.fullName;
                    g.append("g")
                      .classed("rpd-nodelist-nodetype", true)
                      .attr("transform", "translate(0," + lastY + ")")
                      .call(function (g) {
                        var hasDescription = nodeDescriptions[nodeType]
                          ? true
                          : false;
                        ...
                        g.append("rect")
                          .attr("class", "rpd-nodelist-item-bg")
                          .attr("x", 0)
                          .attr("y", -5)
                          .attr("rx", 5)
                          .attr("ry", 5)
                          ...
                          .attr(
                            "height",
                            (hasDescription ? lineHeight * 1.5 : lineHeight) - 5
                          );
                        g.append("text")
                          .attr("class", "rpd-nodelist-icon")
                          .text(nodeTypeIcons[nodeType] || " ")
                          .attr("x", iconWidth / 2)
                          .attr("y", 5);
                        g.append("text")
                          .attr("class", "rpd-nodelist-fulltypename")
                          .attr(
                            "transform",
                            "translate(" + (iconWidth + 4) + ",0)"
                          )
                        ...
                        if (hasDescription) {
                          ...
                          g.append("text")
                            .attr("class", "rpd-nodelist-description")
                            .attr(
                              "transform",
                              "translate(3," + lineHeight * 0.6 + ")"
                            )
                        ...
                        listElementsIdxByType[nodeType] =
        ...
        createClearSearchButton: function () {
          searchGroup
            .append("rect")
            .attr("transform", "translate(" + (nodeListSize.width - 32) + ",7)")
            .attr("width", 12)
            .attr("height", 12)
            .attr("rx", 5);
          return searchGroup
            ...
            .attr(
              "transform",
              "translate(" + (nodeListSize.width - 26) + ",12)"
            );
        },
        clearSearchInput: function (searchInput) {
          searchInput.node().value = "";
        },
        markSelected: function (elmData) {
          elmData.element.classed("rpd-nodelist-selected", true);
        },
        markDeselected: function (elmData) {
          elmData.element.classed("rpd-nodelist-selected", false);
        },
        markAdding: function (elmData) {
          elmData.element.classed("rpd-nodelist-add-effect", true);
        },
        markAdded: function (elmData) {
          elmData.element.classed("rpd-nodelist-add-effect", false);
        },
        setVisible: function (elmData) {
          elmData.element.style("display", "list-item");
        },
        setInvisible: function (elmData) {
          elmData.element.style("display", "none");
        },
      });
