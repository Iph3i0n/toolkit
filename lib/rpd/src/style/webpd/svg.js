      ...
      return function (config) {
        ...
        function redistributeOutlets(node) {
          ...
          outlets.forEach(function (outlet, idx) {
            var xPos =
              idx * (bodyWidth / (outlet_count - 1)) -
              idx * (connectorWidth / (outlet_count - 1));
            outletToElm[outlet.id].attr(
              "transform",
              "translate(" + xPos + ",0)"
            );
          });
        ...
        return {
          ...
          createNode: function (node, render, description, icon) {
            ...
            nodeElm
              .append("g")
              .classed("rpd-process", true)
              .classed("rpd-drag-handle", isPdNode);
            nodeElm.append("g").classed("rpd-inlets", true);
            nodeElm
              .append("g")
              .classed("rpd-outlets", true)
          ...
          getInletPos: function (inlet) {
            var connectorPos = getPos(
              inletToElm[inlet.id].select(".rpd-connector").node()
            );
          ...
          getOutletPos: function (outlet) {
            var connectorPos = getPos(
              outletToElm[outlet.id].select(".rpd-connector").node()
            );
            return { x: connectorPos.x + 3, y: connectorPos.y + 1 };
          },
          ...
          onInletRemove: function (inlet) {
            ...
            nodeToInlets[nodeId] = nodeToInlets[nodeId].filter(function (
              other
            ) {
          ...
          onOutletRemove: function (outlet) {
            ...
            nodeToOutlets[nodeId] = nodeToOutlets[nodeId].filter(function (
              other
            ) {
