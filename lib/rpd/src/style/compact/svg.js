        ...
        return {
          ...
          createNode: function (node, render, description, icon) {
            ...
            function findBestNodeSize(numInlets, numOutlets, minContentSize) {
              var requiredContentWidth =
                2 * socketsMargin +
                (Math.max(numInlets, numOutlets) - 1) * socketPadding;
              return {
                width:
                  headerWidth +
                  Math.max(requiredContentWidth, minContentSize.width),
                height: minContentSize.height,
              };
            }

            var initialSize = findBestNodeSize(
              node.def.inlets ? Object.keys(node.def.inlets).length : 0,
            ...
            nodeElm
              .append("g")
              .attr("class", "rpd-name-holder")
              .attr(
                "transform",
                "translate(3, " + (height + 2) + ") rotate(-90)"
              )
              .append("text")
              .attr("class", "rpd-name")
              .text(node.def.name || "")
            ...
            nodeElm
              ...
              .text(
                description ? description + " (" + node.type + ")" : node.type
              );
            ...
            var processNode = nodeElm
              .append("g")
              .attr("class", "rpd-process")
              .attr(
                "transform",
                "translate(" +
                  (headerWidth + width * pivot.x) +
                  "," +
                  pivot.y * height +
                  ")"
              );
            ...
            function checkNodeSize() {
              ...
              nodeElm
                ...
                .attr("width", newSize.width);
              nodeElm
                .select("rect.rpd-content")
                .attr("width", newSize.width - headerWidth);
              nodeElm
                .select("g.rpd-process")
                .attr(
                  "transform",
                  "translate(" +
                    (headerWidth + (newSize.width - headerWidth) * pivot.x) +
                    "," +
                    newSize.height * pivot.y +
                    ")"
                );
              nodeElm
                .select(".rpd-remove-button")
                .attr("transform", "translate(" + (newSize.width - 12) + ",1)");
              lastSize = newSize;
            }
