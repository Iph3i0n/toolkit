  ...
  var exportSpec = {
    ...
    "patch/open": function (update) {
      return update.parent
        ? ["patch/open", update.patch.id, update.parent.id]
    ...
    "patch/add-node": function (update) {
      ...
      return [
        ...
        node.id,
        node.type,
        encodeURIComponent(node.def.title),
      ];
    },
    ...
    "node/add-inlet": function (update) {
      ...
      return [
        ...
        inlet.type,
        inlet.alias,
        encodeURIComponent(inlet.def.label),
      ];
    },
    ...
    "node/add-outlet": function (update) {
      ...
      return [
        ...
        outlet.type,
        outlet.alias,
        encodeURIComponent(outlet.def.label),
      ];
    },
    ...
    "node/configure": function (update) {
      return ["node/configure", update.node.id].concat(
        JSON.stringify(update.props)
      );
    },
  ...
  function makeImportSpec() {
    ...
    return {
      "network/add-patch": function (command) {
        patches[command[0]] = Rpd.addClosedPatch(
          decodeURIComponent(command[1])
        );
      },
      "patch/open": function (command) {
        patches[command[0]].open(
          command.length > 1 ? patches[command[1]] : null
        );
      },
      ...
      "patch/move-canvas": function (command) {
        patches[command[0]].moveCanvas(
          parseFloat(command[1]),
          parseFloat(command[2])
        );
      },
      "patch/resize-canvas": function (command) {
        patches[command[0]].resizeCanvas(
          parseFloat(command[1]),
          parseFloat(command[2])
        );
      },
      "patch/add-node": function (command) {
        nodes[command[1]] = patches[command[0]].addNode(
          command[2],
          decodeURIComponent(command[3])
        );
      },
      ...
      "node/add-inlet": function (command) {
        inlets[command[1]] = nodes[command[0]].addInlet(
          command[2],
          command[3],
          decodeURIComponent(command[4])
        );
      },
      ...
      "node/add-outlet": function (command) {
        outlets[command[1]] = nodes[command[0]].addOutlet(
          command[2],
          command[3],
          decodeURIComponent(command[4])
        );
