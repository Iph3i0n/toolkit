module.exports = function (Rpd) {
  ...
  Rpd.import.pd = function (lines) {
    ...
    function popInlet(update) {
      nodeToInlets[update.node.id].pop();
    }
    function popOutlet(update) {
      nodeToOutlets[update.node.id].pop();
    }

    function eventIs(type) {
      return function (event) {
        return event.type === type;
      };
    }

    var addInletStream = rootPatch.events
      .filter(eventIs("node/add-inlet"))
      .filter(function (event) {
        return !event.inlet.hidden;
      });
    var addOutletStream = rootPatch.events.filter(eventIs("node/add-outlet"));
    var removeInletStream = rootPatch.events.filter(
      eventIs("node/remove-inlet")
    );
    var removeOutletStream = rootPatch.events.filter(
      eventIs("node/remove-outlet")
    );

    addInletStream.onValue(pushInlet);
    addOutletStream.onValue(pushOutlet);
    removeInletStream.onValue(popInlet);
    removeOutletStream.onValue(popOutlet);
    ...
    patchData.nodes.forEach(function (pdNode, idx) {
      ...
      model.markResolvedAndApply(
        node,
        proto,
        pdNode.args,
        webPdPatch.objects[idx]
      );
      nodes.push(node);
    });
    ...
    patchData.connections.forEach(function (connection) {
      ...
      if (nodeToOutlets[fromNode.id] && nodeToInlets[toNode.id]) {
        ...
        if (!inlet.allows(outlet)) {
          ...
          console.warn(
            "Different types of channels were encountered while connecting",
            "from",
            nodeToProto[fromNode.id],
            "outlet",
            outletIdx,
            outlet.type,
            "to",
            nodeToProto[toNode.id],
            "inlet",
            inletIdx,
            inlet.type,
        ...
        console.error(
          "Failed to connect object " + fromNodeIdx + " to object " + toNodeIdx
        );
      }
    });

    addInletStream.offValue(pushInlet);
    addOutletStream.offValue(pushOutlet);
    removeInletStream.offValue(popInlet);
    removeOutletStream.offValue(popOutlet);
