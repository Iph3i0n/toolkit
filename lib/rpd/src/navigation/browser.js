module.exports = function (Rpd) {
  Rpd.navigation = (function () {
    ...
    Navigation.prototype.enable = function () {
      ...
      this.openedPatchesStream = Rpd.events
        .filter(function (event) {
          return event.type === "patch/open";
        })
        .map(function (event) {
          return event.patch;
        });
      this.openedPatchesStream.onValue(this.onOpenedPatch);
      this.closedPatchesStream = Rpd.events
        .filter(function (event) {
          return event.type === "patch/close";
        })
        .map(function (event) {
          return event.patch;
        });
      this.closedPatchesStream.onValue(this.onClosedPatch);
      this.hashChangeStream = Kefir.fromEvents(window, "hashchange").map(
        function () {
